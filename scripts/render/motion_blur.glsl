...
// displacement since the previous rendered frame, and integrates the color
...
//
// The gather is centered on the pixel rather than trailing behind it. A physical
// shutter closes at the frame time and so blurs only backwards, but a backwards
// blur reads as lag on a camera that is following the action. Centering costs
...
// implementations: a gather can only collect color that is already on screen at
...
layout(rgba16f, set = 0, binding = 2) uniform restrict writeonly image2D dest_color;
...
void main() {
	...
	if (params.blit_only != 0) {
		imageStore(dest_color, pixel, texture(source_color, uv));
		return;
	}
	...
	if (params.debug_mode == DEBUG_VELOCITY) {
		vec2 normalized = velocity_pixels / max(params.max_blur_pixels, 1.0);
		imageStore(dest_color, pixel, vec4(normalized * 0.5 + 0.5, 0.5, 1.0));
		return;
	}
	// Writes uncorrelated noise every frame. If any temporal pass ran *after*
	// this effect it would average the noise towards flat gray; if this effect
	// is the last thing to touch the color buffer, the noise survives to the
	...
	for (int i = 0; i < taps; i++) {
		// (i + 0.5) / taps spans (0, 1); minus 0.5 centers it on the pixel. The
		...
		accumulated += texture(source_color, tap_uv);
	}

	imageStore(dest_color, pixel, accumulated / float(taps));
}
