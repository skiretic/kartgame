...
##
## The other two have **not** been migrated in this pass and that is on purpose:
## `proving_ground.tscn` is where every §6.4 figure is measured and `drive.sh`
## re-measures them on every vehicle change, so moving its spawn plumbing is a
## change to the instrument in the same commit as a change to the thing being
## measured. It wants its own commit and its own `drive.sh` run. Issue #183.
##
## Anything below that says "unchanged" is verbatim from `test_track.gd`, including
## the reasoning, so that a reader comparing them finds them identical rather than
## subtly different.
...
## Spin and steer the wheel meshes. `test_track.gd`'s, unchanged.
func update_wheel_visuals() -> void:
	if kart == null or wheel_pivots.size() != WHEEL_PIVOT_NAMES.size():
		return
	var report: Array = kart.wheel_report()
	for index in mini(report.size(), wheel_pivots.size()):
		var wheel: Dictionary = report[index]
		var pivot := wheel_pivots[index]
		pivot.rotation.y = wheel_rest_yaw[index] + float(wheel.get("steer", 0.0))
		pivot.rotation.x = float(wheel.get("spin", 0.0))
