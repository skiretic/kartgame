...
## scene and `proving_ground.gd` and `circuit.gd` build the same kart rather than
## three karts that drifted apart — which is what they were. Issue #183.
...
func _build_kart() -> void:
	...
	_spawn = (grid["position"] as Vector3) + Vector3(
		0.0, TrackRibbon.ROAD_LIP + KartRig.SPAWN_LIFT, 0.0
	)
	...
	# and the listener — `KartRig` owns all of it and this scene owns where the grid
	# is. The scripted-input branch is in there too; `AssistSettings.apply` below
	# decides on `_args` rather than on whether `input_driver` has been set, so the
	# order of the two is not load-bearing.
	_kart = _rig.build(self, Transform3D(basis, _spawn), _args)
	_driver = _rig.driver
...
## The surface read-out and the steered front wheels, off one `wheel_report()`.
##
## `KartRig` turns the wheels — its header records that the extraction read the
## wrong key for a milestone — and this keeps the report itself because the surface
## overlay needs it too. `wheel_report()` allocates an Array of four Dictionaries,
## and `driving_hud.gd`'s header makes the argument in its own words: an allocation
## is fine for a panel that samples while it is open and wrong for an overlay that
## draws every frame. So the report is taken once here and handed to the rig, rather
## than the rig fetching a second one.
func _update_wheel_visuals() -> void:
	if _kart == null:
		return
	var report: Array = _kart.wheel_report()
	_surface_text = surface_line(report)
	_rig.update_wheel_visuals_from(report)
