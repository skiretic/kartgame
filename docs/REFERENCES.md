...
| F7 | **The published FIA Karting result documents for a real 2026 event** — KZ2 entry list (83 drivers), Qualifying Practice classification (Document 13.2 OFFICIAL), Final classification, and the 199-page results booklet with per-driver Lap Time Analysis. Fetched by a browser session; files in `refs/frontend/` (gitignored) with `sources.txt` | `nocache.fiakarting.com/sites/default/files/xml_folder/2026/Karting%20Genk/` — `kz2_entry.pdf`, `kz2_resultsNN.pdf` (NN = session id), `Booklet_KZ2.pdf`. © FIA/RGMMC — **reference only** |
| F8 | The live-timing page captured idle: black grid area, navy chrome, track map drawn as a green ribbon with white sector-boundary ticks, green CONNECTED pill, local-time footer | `refs/frontend/apex_fiakarting_live.png` (gitignored), same source as F6 |
