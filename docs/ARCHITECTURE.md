...
| Vehicle tuning eats unbounded time | Telemetry lands *with* the vehicle (M3b), never after. Replay-based A/B. Timebox and move on. **And tuning is auditable rather than unbounded** — [ADR-0037](DECISIONS.md#adr-0037--tuning-is-an-audit-trail-that-happens-to-be-adjustable-and-a-preset-is-not-in-the-state-hash): every tunable declares whether its default is sourced, measured, derived or a guess; a preset is a diff against those defaults, so "what has been tuned away from its source, and by how much" is one command; overriding a sourced or measured default needs an explicit acknowledgement and is marked in the saved file. This row named one defense until M3b and now names two, because telemetry answers *what the kart is doing* and says nothing about *what has been changed to make it do that*. |
