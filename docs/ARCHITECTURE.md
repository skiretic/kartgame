...
| Layer | Language | Why |
|---|---|---|
| Vehicle model, tyre solver, gearbox, substepping | **C++ GDExtension** | Tight numeric loop at 120–240 Hz. GDScript is too slow and too imprecise here. |
...
| Track visual mesh, kerbs, lightmap UVs | **Blender Python** | Same reasoning. Reads `track.json`. |
...
1. **Correct real-world scale.** Godot is 1 unit = 1 metre. A kart is ~1.05 m wheelbase, ~1.4 m track width, ~0.28 m tall at the frame. Get this wrong and nothing else can save it — the brain reads scale before it reads shading.
...
3. **Photoscan materials.** CC0 scanned asphalt, concrete, kerb paint, rubber. Real measured albedo and roughness beat anything hand-authored or generated.
...
7. **Surface history.** Skid decals, marbles off the racing line, dirt at track edges, kerb paint scuffed where cars actually clip it.
8. **Triplanar detail textures** on the track for close-up asphalt grain under the cockpit camera.
9. **Real kart reference data.** Mass ~180 kg with driver, real torque curve, real tyre compound behaviour. §6.
...
| Wheels | Per-wheel raycast from mount point; shapecast where kerb robustness demands it |
| Suspension | Spring + damper, split bump/rebound, travel limits, plus the flex and jacking terms above |
| Tyres | Simplified Pacejka: lateral by slip angle, longitudinal by slip ratio, combined through a friction ellipse, grip falling with normal load |
...
| Surfaces | asphalt / kerb / grass / dirt → grip multiplier, roughness, particle and audio hooks |
...
- Over-revving on a downshift is a real mistake worth modelling
...
`_physics_process` at **120 Hz** (`physics/common/physics_ticks_per_second`), with the vehicle solver substepping internally to **240 Hz**. Tyre models go unstable at 60 Hz. This is not tunable-down.
...
Adaptive triggers are worth real consideration despite the cost: R2 resistance rising with engine load, L2 gaining a hard detent at lock-up threshold. That puts the tyre model in your finger instead of on the HUD, and it surfaces §6 better than any UI could. Scheduled as its own milestone so it can be cut without disturbing anything else.
...
| Fab free tier (Megascans) | Scanned materials and props | **Check the licence before use** — Epic's terms are not CC0 and have carried engine restrictions |

Prefer CC0 sources. For an open source project, licence clarity is worth more than a marginally better scan. Record provenance for every imported asset in `ATTRIBUTION.md`, even for CC0 where it is not legally required.
...
| `gentrack.py` | Reads `track.json`. Track surface, kerbs, run-off, barriers, UV-per-metre, lightmap UV2, LODs, glTF. |
...
```
track.json  ──┬──►  gentrack.py (Blender)  ──►  visual mesh, kerbs, UVs, LODs  ──► glTF
...
- Elevation change, both for looks and because it loads the tyres
...
- **Tyre scrub:** filtered noise modulated by slip magnitude — falls straight out of §6 for free.
...
kartgame/
  ...
  src/                      C++ GDExtension (SCons + godot-cpp)
    vehicle/                chassis, suspension, tyres, gearbox, clutch
...
| Layer | Approach |
|---|---|
| C++ units | doctest or Catch2 on the vehicle math, tyre curves, spline solver — engine-independent, fast |
...
| CI | GitHub Actions: build the extension for macOS/Windows/Linux, run C++ units and the determinism harness headless |
...
- **Not yet a git repo.** `git init` is step zero of M0.
...
- **Licence split:** code under MIT or Apache-2.0; assets under CC0 or CC-BY as their sources require. State both in `README.md`.
- `ATTRIBUTION.md` listing every third-party asset with its source and licence. Do this continuously — reconstructing provenance later is miserable.
...
- Colourblind-safe HUD, scalable UI
...
| Asset licence contamination | `ATTRIBUTION.md` updated at import time. Prefer CC0 sources over marginally better restricted ones. |
